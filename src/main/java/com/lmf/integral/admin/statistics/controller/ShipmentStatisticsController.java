 ...
 */
@Controller
public class ShipmentStatisticsController {

    @Autowired
    private OrderShipmentLogService orderShipmentLogService;

    @RequestMapping(value = "/admin/statistics/shipmentProductStatistics.php", method = RequestMethod.GET)
    public String list(@RequestParam(value = "timeType", required = false) String timeType,
        ...
        StringBuilder link = new StringBuilder("/jdvop/admin/statistics/shipmentProductStatistics.php?page=[:page]");
        ...
        if (timeType != null && timeType.equals("shipmentTime")) {
            model.addAttribute("orderSeparationEntrys", orderShipmentLogService.findShipmentProducts(new OrderShipmentLogCriteria().withShipmentTime(new Range<>(startDate, endDate)), pager));
        } else {
            model.addAttribute("orderSeparationEntrys", orderShipmentLogService.findShipmentProducts(new OrderShipmentLogCriteria().withConfirmTime(new Range<>(startDate, endDate)), pager));
        }
        link.append("&timeType=").append(timeType);
        model.addAttribute("timeType", timeType);
        model.addAttribute("link", link.toString());

        return "admin/statistics/shipment_product";
    }
    
    @RequestMapping(value = "/admin/statistics/shipmentexport.php", method = RequestMethod.GET)
    public @ResponseBody void exportAsExcel(@RequestParam(value = "timeType", required = false) String timeType,
                                            @RequestParam(value = "st", required = false) Date startDate,
                                            @RequestParam(value = "et", required = false) Date endDate,
        ...
        sbd.append(website.getName()).append("发货单明细");
        ...
        try (OutputStream out = response.getOutputStream()) {
            ...
            WritableSheet sheet = workbook.createSheet("导出发货单", 0);
            ...
            Page<OrderSeparationEntry> shipmentLog;
            if ("shipmentTime".equals(timeType)) {
                shipmentLog = orderShipmentLogService.findShipmentProducts(new OrderShipmentLogCriteria().withShipmentTime(new Range<>(startDate, endDate)), null);
            } else {
                shipmentLog = orderShipmentLogService.findShipmentProducts(new OrderShipmentLogCriteria().withConfirmTime(new Range<>(startDate, endDate)), null);
            }
            if (shipmentLog != null) {
                int row = 1;
