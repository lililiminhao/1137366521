...
public class PagerTool {
    ...
    public static String pager(String url, PagerSpec ps, int delta, String target)
    {
        ...
        sb.append("<div class='multipage'><em>共(")
        ...
        if (currentPage > 1)
        {
            sb.append("<a href='")
                    ...
                    .append("' target='")
                    .append(target)
                    .append("' class='link h-bn h-page'>首页</a>");
            sb.append("<a href='")
                    .append(url.replace("[:page]", String.valueOf(currentPage - 1)))
                    .append("' class='link p-bn p-page' target='")
                    .append(target)
                    .append("'>前一页</a>");
        } else {
            sb.append("<a class='no-page h-page'>首页</a>");
            sb.append("<a class='no-page p-page'>前一页</a>");
        }
        ...
        if (currentPage < totalPage)
        {
            sb.append("<a href='")
                    .append(url.replace("[:page]", String.valueOf(currentPage + 1)))
                    .append("' class='link n-bn n-page' target='")
                    .append(target).append("'>后一页</a>");
            
            sb.append("<a href='")
                    .append(url.replace("[:page]", String.valueOf(totalPage)))
                    .append("' class='link e-bn e-page' target='")
                    .append(target)
                    .append("'>尾页</a>");
        } else {
            sb.append("<a class='no-page n-page'>后一页</a>");
            sb.append("<a class='no-page e-page'>尾页</a>");
        }
        sb.append("</div>");
