...
$(function(){
    ...
    $(document).on('click', 'li.js-add-notice', function() {
        ...
        $.get('/jdvop/admin/custom/modifyText.php?type=create', data, function(data){
            if(data.isOk) {
    ...
    $(document).on('click', 'li.js-add-slider', function() {
        ...
        serial_num ++;
        $.get('/jdvop/admin/custom/modifySlider.php?type=create', data, function(data){
            if(data.isOk) {
    ...
    $(document).on('click', 'li.js-add-search', function() {
        ...
        $.get('/jdvop/admin/custom/modifySearchBox.php', data, function(data){
            if(data.isOk) {
    ...
    $(document).on('click', 'li.js-add-product', function() {
        ...
        $.get('/jdvop/admin/custom/modifyProductBox.php', data, function(data){
            if(data.isOk) {
...
/*========================================================== 模板内容end ==================================================================*/
    ...
    var image_count;
    var cates;
    $.get("/jdvop/admin/custom/cates.php", {}, function(data) {
    ...
    $(document).on("click", 'span.js-btnEdit', function(){
        ...
        if(key === "notice") {
            ...
            var sliderHTML = '<img src="/jdvop/images/admin/ico-arrow2.png" class="img-arrow2"/>';
                ...
                sliderHTML += '</div>';
                if(key === "slider") {
                    $.get("/jdvop/admin/custom/getCustomBlocks.php", {id: blockId}, function(d) {
                        if (typeof d === 'object')
                        {
                            var appendHTML = '';
                    ...
                    $.get("/jdvop/admin/custom/getCustomBlock.php", {id: blockId}, function(d) {
                        if (typeof d === 'object')
                        {
...
/*========================================================== 编辑模板end ==================================================================*/
    ...
    $(document).on("click", 'span.js-btnDel', function(){
        ...
        if(blockId !== "") {
            $.get("/jdvop/admin/custom/delete.php", {id: blockId}, function(d) {
                if (!d.isOk)  {
...
/*---------------------------------图片链接切换star-----------------------------------------------*/
    $(document).on("change", "#js-imgLink", function() {
        ...
        if(thiz.val() === "cate") {
            ...
            if(len === 0) {
                $.get("/jdvop/admin/custom/cates.php", {}, function(d) {
                    if (typeof d === 'object')
                    {
...
/*---------------------------------图片链接切换end-----------------------------------------------*/
    $(document).on("change", ".js-child-cates", function(){
        ...
        if(cateId > 0) {
            $.get("/jdvop/admin/custom/childCates.php", {cateId: cateId}, function(data) {
                if (typeof data === 'object')
                {
                    var len = data.length;
    ...
    $(document).on("click", ".js-upload", function() {//上传图片
        ...
        var    key = $(this).attr('rev');
        var height = 300;
        _fileuploadURL = "/jdvop/admin/custom/upload.php?width=640&height=" + height + "&type=" + key;
        $('#file_upload').click();
    });
    ...
    $(document).on("click", '.js-save-btn', function(){
        ...
        if (blockType === "text") {
            ...
            $.get('/jdvop/admin/custom/modifyText.php?type=edit', $('#form2').serialize(), function(data){
                if(data.isOk) {
            ...
            if(isOk) {
                $.get('/jdvop/admin/custom/modifySlider.php?type=edit', $('#form2').serialize(), function(data){
                    if(data.isOk) {
           ...
           if(model_title == '')
            ...
            $.get('/jdvop/admin/custom/modifyProductBox.php?type=edit&backImag='+bgImage+(spacialActivityId?('&specialActivityId='+spacialActivityId):''), $('#form2').serialize(), function(data){
                if(data.isOk) {
            ...
            $.get('/jdvop/admin/custom/modifyImage.php?type=edit', $('#form2').serialize(), function(data){
                if(data.isOk) {
    ...
    $(document).on("click", '.js-submit-btn', function(e){
        ...
        $.post($('#form1').attr('action'), $('#form1').serialize(), function(data){
            if(data.isOk) {
                ...
                if(url === "/jdvop/admin/custom/save.php"){
                    top.artAlert("温馨提示", "您的配置保存成功！", 'ok');
                    location = "/jdvop/admin/custom.php"; //首页保存跳转路径
